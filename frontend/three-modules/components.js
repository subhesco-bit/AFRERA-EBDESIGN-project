window.AfreraUI = (function () {
  ...
  function card(title, bodyNodes, footer) {
    ...
    (Array.isArray(bodyNodes) ? bodyNodes : [bodyNodes]).forEach((b) => c.appendChild(typeof b === 'string' ? el('p', { text: b }) : b));
  ...
  function gauge(label, value0to100) {
    ...
    return el('div', {}, [
      ...
      el('div', { className: 'gauge-wrap', role: 'meter', 'aria-valuenow': String(v), 'aria-valuemin': '0', 'aria-valuemax': '100' }, [
  ...
  function crossLinks(fromId) {
    const map = {
      veterinary: [
        { href: '#/nutrition', label: 'Farm family nutrition (zoonotic link)' },
        { href: '#/unified', label: 'Unified One Health operate' },
      ...
      agro: [
        ...
        { href: '#/unified', label: 'Farm continuum' },
      ],
      unified: [
