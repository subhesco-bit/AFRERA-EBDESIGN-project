...
export default function AdvancedMedicalCodingPage() {
  ...
  const suggestion = useMutation({
    mutationFn: () => api.post('/advanced-medical-coding/ai-coding-assistance', { clinicalDescription, codeSystem }).then((response) => response.data),
  });
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="rounded-[2rem] bg-gradient-to-br from-blue-950 via-slate-950 to-emerald-950 p-7 text-white shadow-xl sm:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">Supports farmer occupational-health encounters, telehealth documentation, insurance claims, pre-authorisation, billing, ERP finance, and audit. AI produces draft candidates only; it never diagnoses, binds coverage, or submits a claim.</p>
            </div>
        ...
        <section className="mt-7 grid gap-4 md:grid-cols-3">
          {[
            ...
            [Activity, 'Insurer and ERP', 'Validated claim data, finance posting, utilisation reporting, and immutable audit events.'],
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_0.9fr]">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-4">
              ...
              <label className="block"><span className="mb-1.5 block text-sm font-semibold text-slate-800">Clinical documentation</span><textarea value={clinicalDescription} onChange={(event) => setClinicalDescription(event.target.value)} rows={8} placeholder="Example: clinician-authored symptoms, confirmed findings, assessment, procedure details, laterality, encounter context…" className="w-full rounded-xl border border-slate-300 px-3 py-2.5 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100" /></label>
            ...
            {result && <div className="mt-6 space-y-4 border-t border-slate-200 pt-6">
              <div className="flex flex-wrap items-center gap-2"><Pill tone={result.source === 'openai-draft' ? 'blue' : 'amber'}>{result.source}</Pill><Pill tone="amber">human review required</Pill><Pill>{result.code_system}</Pill>{result.cached && <Pill tone="green">cached</Pill>}</div>
