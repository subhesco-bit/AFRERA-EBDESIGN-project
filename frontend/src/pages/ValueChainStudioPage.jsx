            ...
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <Section icon={Landmark} title="Funding readiness" provenance={provenance} path="funding.applications" link="/loan-management">
                {plan.funding?.summary ? (
                  <div className="text-sm text-slate-700">
                    ...
                    <div className="mt-1">Requested: <strong>{fmtMoney(plan.funding.summary.totalRequested)}</strong> · Approved: <strong>{fmtMoney(plan.funding.summary.totalApproved)}</strong></div>
                    <ProvenanceBadge provenance={provenance} path="funding.summary" />
                  </div>
                ) : <EmptyNote>No funding data for this farmer context.</EmptyNote>}
                {plan.funding?.applications?.length > 0 && (
                  <ul className="mt-3 space-y-1 text-xs text-slate-600">
                    {plan.funding.applications.slice(0, 5).map((a) => (
                      <li key={a.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-2 py-1">
                        <span>{a.loan_type || 'Loan'} · {fmtMoney(a.amount_requested)}</span>
              ...
              <Section icon={Truck} title="Logistics" provenance={provenance} path="logistics.shipments" link="/logistics">
                {plan.logistics?.shipments?.length > 0 ? (
                  <ul className="space-y-2 text-sm text-slate-700">
                    {plan.logistics.shipments.map((s) => (
                      <li key={s.id} className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
                        <span>{s.tracking_number || s.id} · {s.origin} → {s.destination}</span>
                        <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-bold uppercase">{s.status}</span>
                      </li>
                    ))}
                  </ul>
                  ...
                  {positioning.data && (
                    <div className="mt-3 rounded-xl bg-violet-50 p-3 text-sm text-violet-900">
                      {positioning.data.copy
                        ? <pre className="whitespace-pre-wrap font-sans text-sm">{positioning.data.copy}</pre>
                        : <span>{positioning.data.provenance?.note || 'No AI provider is configured.'}</span>}
                    </div>
                  )}
                </div>
                <div>
