...
async function checkInventoryAvailability(items) {
  ...
  for (const item of items) {
    let res = await pool.query('SELECT quantity FROM products WHERE product_id = $1', [item.product_id]);
    availability.push({
...
async function getOrderTracking(orderId) {
  let res = await pool.query('SELECT * FROM order_tracking WHERE order_id = $1 ORDER BY timestamp DESC', [orderId]);
  return res.rows;
}
