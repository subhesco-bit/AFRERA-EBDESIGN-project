...
function make(cfg){
 ...
 async function create(data,actor={}){validate(data);const id=crypto.randomUUID();const fields=['id',...cols,'created_by'];const vals=[id,...cols.map(k=>data[k]??null),actor.id||null];const marks=vals.map((_,i)=>`$${i+1}`);const q=await pool.query(`INSERT INTO ${cfg.table}(${fields.join(',')}) VALUES(${marks.join(',')}) RETURNING *`,vals);return q.rows[0];}
