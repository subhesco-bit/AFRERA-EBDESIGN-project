// Service for M034 Module
const { logger } = require('../../utils/logger');
const { getPostgreSQL } = require('../../database/connection');

const tableName = 'm034_items';
...
async function createItem(payload) {
  ...
  logger.info('Created item in M034: ' + res.rows[0].id);
  return res.rows[0];
}
...
async function updateItem(id, payload) {
  ...
  if (res.rows[0]) {
    logger.info('Updated item in M034: ' + id);
  }
...
async function deleteItem(id) {
  ...
  if (res.rows[0]) {
    logger.info('Deleted item in M034: ' + id);
  }
