...
router.get('/procurement-plans/:id', authenticate, async (req, res) => {
  try {
    ...
    let result = { plan: null, message: 'Procurement plan details retrieval' };
...
router.post('/subscriptions', authenticate, async (req, res) => {
  try {
    let result = await service.createSubscription(req.body);
...
router.put('/subscriptions/:id', authenticate, async (req, res) => {
  try {
    let result = await service.manageSubscription(req.params.id, req.body);
...
router.get('/dashboard', authenticate, requireRole('household'), async (req, res) => {
  try {
    let result = await service.getHouseholdDashboard(req.user.id);
...
router.post('/aggregate-orders', authenticate, authorize(['admin', 'logistics']), async (req, res) => {
  try {
    let result = await service.aggregateHouseholdOrders(req.body.region, req.body.delivery_date);
...
router.get('/aggregation-groups/:id', authenticate, authorize(['admin', 'logistics']), async (req, res) => {
  try {
    ...
    let result = { group: null, message: 'Aggregation group details retrieval' };
