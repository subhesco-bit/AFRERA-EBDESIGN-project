'use strict';
/** GI token path, herd heads, nested remaining. Repair names the break.
 * Ported from pine-shadow src/lib/vet/lineage.ts */

const { nestedRemaining } = require('../erp/atlas');

const GI_ORDER = ['mint', 'intake', 'settle'];

function expectedGi(lot) {
  const gi = Boolean(lot.giMinted || lot.giMarker);
  if (lot.status === 'settled' && gi) return ['mint', 'intake', 'settle'];
  if ((lot.status === 'in_warehouse' || lot.status === 'pledged' || lot.status === 'listed') && gi) return ['mint', 'intake'];
  if (gi) return ['mint'];
  return [];
}

function massBroken(lot) {
  const broken = [];
...
function giLotLineage(lot, chain) {
  ...
  for (let i = 1; i < path.length; i++) {
    const prev = GI_ORDER.indexOf(path[i - 1]);
    const next = GI_ORDER.indexOf(path[i]);
  ...
  return { kind: 'gi-lot', conserved, broken, path: path.length ? path : expected, remaining: lot.remainingGrams, rupee: null, reason };
}

function repairedGiPath(lot, chain) {
  const report = giLotLineage(lot, chain);
  if (report.conserved) return report.path;
  return expectedGi(lot);
}

function herdCellLineage(herd, cells) {
  ...
  const conserved = broken.length === 0;
  return {
    kind: 'herd-cell', conserved, broken, path, remaining, rupee: null,
    reason: conserved
...
function nestedLineage(cells, lots) {
  ...
  for (const cell of cells) {
    ...
    if (lotSum !== cell.remainingGrams) broken.push(`${cell.id}: cell remaining ${cell.remainingGrams} g ≠ lot sum ${lotSum} g.`);
  }
  const conserved = broken.length === 0;
  return {
    kind: 'person-home-village', conserved, broken, path: ['person', 'home', 'village'],
    remaining: nested.village.remainingGrams, rupee: null,
    reason: conserved
...
function repairLineages(input) {
  const lots = input.lots ?? [];
  const chain = input.giChain ?? [];
  const herd = input.herd ?? [];
  const cells = input.cells ?? [];
  const reports = [...lots.map((lot) => giLotLineage(lot, chain)), herdCellLineage(herd, cells), nestedLineage(cells, lots)];
  const broken = reports.flatMap((r) => r.broken);
  const repaired = lots.filter((lot) => !giLotLineage(lot, chain).conserved).map((lot) => `${lot.id} expects ${repairedGiPath(lot, chain).join(' → ') || 'no GI token'}`);
  return { reports, conserved: broken.length === 0, broken, repaired, rupee: null, livestockCash: 'missing', githubMedical: 'refused', afreraVet: 'living' };
}

module.exports = { giLotLineage, repairedGiPath, herdCellLineage, nestedLineage, repairLineages };
