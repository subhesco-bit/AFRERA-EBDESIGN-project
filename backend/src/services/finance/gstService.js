...
const GST_STATE_ALIASES = Object.freeze({
  AS: 'AS', ASSAM: 'AS', WB: 'WB', 'WEST BENGAL': 'WB', DL: 'DL', DELHI: 'DL',
  HR: 'HR', HARYANA: 'HR', PB: 'PB', PUNJAB: 'PB', UP: 'UP', 'UTTAR PRADESH': 'UP',
  BR: 'BR', BIHAR: 'BR', MH: 'MH', MAHARASHTRA: 'MH', GJ: 'GJ', GUJARAT: 'GJ',
  RJ: 'RJ', RAJASTHAN: 'RJ', KA: 'KA', KARNATAKA: 'KA', KL: 'KL', KERALA: 'KL',
  TN: 'TN', 'TAMIL NADU': 'TN', TS: 'TS', TELANGANA: 'TS', AP: 'AP', 'ANDHRA PRADESH': 'AP',
});
const VALID_GST_STATE_CODES = new Set([...Array.from({ length: 38 }, (_, i) => String(i + 1).padStart(2, '0')), '97']);
const GSTIN_CHARS = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';

function serviceError(message, statusCode = 400, code = 'GST_VALIDATION_ERROR') {
...
function normalizeState(value) {
  ...
  return GST_STATE_ALIASES[raw] || raw;
}

function gstinChecksum(base14) {
  let factor = 2;
  let sum = 0;
  for (let i = base14.length - 1; i >= 0; i -= 1) {
    const codePoint = GSTIN_CHARS.indexOf(base14[i]);
...
class GSTService {
  ...
  getGSTRate(productCategory) {
    ...
    return Object.prototype.hasOwnProperty.call(gstRates, productCategory) ? gstRates[productCategory] : 18;
  }

  calculateTaxComponents({ taxableValue, rate, supplierState, placeOfSupply, reverseCharge = false, exempt = false } = {}) {
    const value = Number(taxableValue);
    let appliedRate = exempt ? 0 : Number(rate);
    if (!Number.isFinite(value) || value < 0) throw serviceError('taxableValue must be a non-negative number');
    if (!Number.isFinite(appliedRate) || appliedRate < 0) throw serviceError('rate must be a non-negative number');
    if (!supplierState || !placeOfSupply) throw serviceError('supplierState and placeOfSupply are required');

    const intraState = normalizeState(supplierState) === normalizeState(placeOfSupply);
    const totalTax = r2((value * appliedRate) / 100);
    const cgst = intraState ? r2(totalTax / 2) : 0;
    const sgst = intraState ? r2(totalTax - cgst) : 0;
    const igst = intraState ? 0 : totalTax;
    return {
      taxableValue: r2(value),
      rate: appliedRate,
  ...
  async resolveEffectiveRate({ hsnCode, supplyDate, productCategory = null } = {}) {
    if (!hsnCode) throw serviceError('hsnCode is required');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(supplyDate || ''))) throw serviceError('supplyDate must be YYYY-MM-DD');
    const result = await this.pool.query(
      `SELECT hsn_code, gst_rate, effective_date, expiry_date, product_category
         FROM gst_rates
        WHERE hsn_code = $1
        ...
        LIMIT 1`,
      [String(hsnCode), String(supplyDate), productCategory],
    );
    if (!result.rows.length) throw serviceError('No effective GST rate found for the requested HSN/date', 404, 'GST_RATE_NOT_FOUND');
    return { ...result.rows[0], gst_rate: Number(result.rows[0].gst_rate), source: 'gst_rates_effective_date' };
  }

  async createReturn(data = {}) {
    const taxpayerGstNumber = String(data.taxpayerGstNumber || data.taxpayer_gst_number || '').toUpperCase();
    const returnPeriod = String(data.returnPeriod || data.return_period || '');
    const returnType = String(data.returnType || data.return_type || 'GSTR3B').toUpperCase();
    const dueDate = data.dueDate || data.due_date;
    if (!this.validateGSTNumber(taxpayerGstNumber)) throw serviceError('A valid taxpayer GSTIN is required');
    if (!/^(0[1-9]|1[0-2])\d{4}$/.test(returnPeriod)) throw serviceError('returnPeriod must be MMYYYY');
    if (!['GSTR1', 'GSTR3B'].includes(returnType)) throw serviceError('Unsupported GST return type');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(String(dueDate || ''))) throw serviceError('dueDate must be explicitly supplied as YYYY-MM-DD');
    const result = await this.pool.query(
      `INSERT INTO gst_returns
         (return_period, return_type, taxpayer_gst_number, taxpayer_name, taxpayer_state, due_date, return_status)
       VALUES ($1,$2,$3,$4,$5,$6,'pending') RETURNING *`,
      [returnPeriod, returnType, taxpayerGstNumber, data.taxpayerName || null, data.taxpayerState || null, dueDate],
    );
  ...
  async createPayment(data = {}) {
    ...
    const taxType = String(data.taxType || data.tax_type || '').toUpperCase();
    if (!Number.isFinite(amount) || amount <= 0) throw serviceError('amount must be greater than zero');
    if (!['CGST', 'SGST', 'IGST', 'CESS'].includes(taxType)) throw serviceError('taxType must be CGST, SGST, IGST or CESS');
    const result = await this.pool.query(
      `INSERT INTO gst_payments
         (return_id, payment_type, tax_type, amount, payment_date, payment_method, transaction_id, challan_number, payment_status, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'pending',$9) RETURNING *`,
      [data.returnId || data.return_id || null, data.paymentType || data.payment_type || 'tax',
        taxType, amount, data.paymentDate || data.payment_date || null, data.paymentMethod || data.payment_method || null,
        data.transactionId || data.transaction_id || null, data.challanNumber || data.challan_number || null,
        data.createdBy || data.created_by || null],
    );
    return result.rows[0];
  }

  async transitionReturn(returnId, nextStatus) {
    const target = String(nextStatus || '').toLowerCase();
    const allowed = {
      pending: new Set(['prepared', 'cancelled']),
      prepared: new Set(['filed', 'cancelled']),
      filed: new Set(['accepted', 'rejected']),
      rejected: new Set(['prepared', 'cancelled']),
      accepted: new Set(),
      cancelled: new Set(),
    };
    if (!Object.prototype.hasOwnProperty.call(allowed, target)) throw serviceError('Unsupported GST return status');
    const client = await this.pool.connect();
    try {
      await client.query('BEGIN');
      const current = await client.query('SELECT * FROM gst_returns WHERE id=$1 FOR UPDATE', [Number(returnId)]);
      if (!current.rows.length) throw serviceError('GST return not found', 404, 'GST_RETURN_NOT_FOUND');
      const from = String(current.rows[0].return_status || '').toLowerCase();
      if (from === target) {
        await client.query('COMMIT');
        return { ...current.rows[0], replayed: true };
      }
      if (!allowed[from] || !allowed[from].has(target)) {
        throw serviceError(`Illegal GST return transition ${from} -> ${target}`, 409, 'GST_RETURN_TRANSITION_CONFLICT');
      }
      const updated = await client.query(
        'UPDATE gst_returns SET return_status=$2, updated_at=NOW() WHERE id=$1 RETURNING *',
        [Number(returnId), target],
      );
      await client.query('COMMIT');
   ...
   */
  async postGSTInvoiceToLedger(client, {
    companyId, invoiceId, invoiceNumber, invoiceDate, actorId = null,
    ...
    const lines = [{ accountId: arAccountId, debit: totalGST, credit: 0, description: `GST receivable - ${invoiceNumber}` }];
    ...
    if (Number(cgstAmount) > 0) {
      ...
      lines.push({ accountId: id, debit: 0, credit: r2(cgstAmount), description: `Output CGST - ${invoiceNumber}` });
    }
    if (Number(sgstAmount) > 0) {
      ...
      lines.push({ accountId: id, debit: 0, credit: r2(sgstAmount), description: `Output SGST - ${invoiceNumber}` });
    }
    if (Number(igstAmount) > 0) {
      ...
      lines.push({ accountId: id, debit: 0, credit: r2(igstAmount), description: `Output IGST - ${invoiceNumber}` });
    }

    const posted = await accounting.createAndPostJournalWithClient(client, {
      companyId,
      entryDate: invoiceDate,
      entryNumber: `GST-${invoiceNumber}`,
      journalType: 'sales',
    ...
    return { posted: true, journalEntryId: posted.id, entryNumber: posted.entry_number, totalGST, replayed: Boolean(posted.replayed) };
  }
  ...
  validateGSTNumber(gstNumber) {
    ...
    if (!VALID_GST_STATE_CODES.has(value.slice(0, 2))) return false;
    return gstinChecksum(value.slice(0, 14)) === value[14];
  }
