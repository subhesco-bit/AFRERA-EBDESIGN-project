...
const TEAM_ROLES = [
  { id: 'lead_architect', title: 'Lead Design Architect', focus: 'Scope, options, coordination of disciplines' },
  { id: 'structural', title: 'Structural Engineer', focus: 'Frame system, loads checklist, foundation interface' },
  { id: 'mep', title: 'MEP Engineer', focus: 'Mechanical, electrical, plumbing design packages' },
  { id: 'agricultural', title: 'Agricultural Infrastructure Engineer', focus: 'Process layout, capacity, agri-specific systems' },
  { id: 'cost_estimator', title: 'Cost Estimator', focus: 'BOQ categories and estimate readiness (links to real rates)' },
  { id: 'compliance', title: 'Compliance & Approvals Engineer', focus: 'Permits, codes checklist, stakeholder gates' },
];
...
function structuralPackage(facilityType, inputs = {}) {
  const packages = {
    cold_storage: [
      'Confirm design loads: dead, live, wind, seismic (IS 875 / IS 1893 — verify locally)',
      'Choose primary system: PEB portal / RCC frame / hybrid with insulated sandwich panels',
      'Insulated panel / PEB / RCC interface and vapour barrier continuity',
      'Floor slab: product racking point loads, forklift aisle loads, cold-room threshold detail',
      'Roof / wall thermal movement and expansion joints at warm–cold interface',
      'Dock leveller and ante-room structural openings',
      'Equipment pads: compressor, condenser, receiver (vibration isolation checklist)',
      'Mezzanine or plant-room if refrigeration plant is elevated',
      'Snow / wind uplift on lightweight roof panels (site wind zone)',
      'Fire compartmentation interfaces with process areas (if multi-use building)',
    ],
    polyhouse: [
  ...
  const hints = { spanM: null, approxBayAreaM2: null, approxEnvelopeVolumeM3: null, targetClearSpanM: null, formulas: {} };

  const length = Number(inputs.lengthM);
  const width = Number(inputs.widthM);
  const height = Number(inputs.heightM);
  ...
  if (
    ...
    hints.approxEnvelopeVolumeM3 = Math.round(length * width * height * 100) / 100;
    hints.formulas.envelopeVolume = 'volume_m3 = length_m × width_m × height_m';
    provenance['structural.hints.approxEnvelopeVolumeM3'] = fromInput();
  } else {
    provenance['structural.hints.approxEnvelopeVolumeM3'] = unavailable(
      'Requires inputs.lengthM, inputs.widthM, and inputs.heightM',
    );
  }

  const clearSpan = Number(inputs.targetClearSpanM);
  if (Number.isFinite(clearSpan) && clearSpan > 0) {
    hints.targetClearSpanM = clearSpan;
    hints.formulas.clearSpan = 'targetClearSpanM is operator-supplied design intent only';
    provenance['structural.hints.targetClearSpanM'] = fromInput();
  } else {
    provenance['structural.hints.targetClearSpanM'] = unavailable('Optional: inputs.targetClearSpanM');
  }
...
function compliancePackage(facilityType, state) {
  ...
  return {
    ...
    provenance: { 'compliance.package': catalog('Compliance checklist catalog') },
  };
}
...
async function buildDesignTeamPlan({
  ...
  const engineers = TEAM_ROLES.map((role) => {
    ...
    return { ...role, status: 'assigned', package: packageData };
  });
