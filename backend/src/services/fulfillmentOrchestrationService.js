...
async function allocate({shipmentId, allocations=[]}){
  ...
  try { await client.query('BEGIN');
    ...
    for(const a of allocations){ if(!(Number(a.quantity)>0)) throw new Error('Allocation quantity must be positive'); await client.query(`INSERT INTO fulfillment_allocations (shipment_id,order_item_id,supply_lot_id,allocated_quantity,unit) VALUES ($1,$2,$3,$4,$5)`,[shipmentId,a.orderItemId,a.supplyLotId||null,a.quantity,a.unit||'kg']); }
    ...
    await client.query(`INSERT INTO delivery_events (shipment_id,event_type,actor_id,notes) VALUES ($1,'allocated',$2,$3)`,[shipmentId,auditActor(),'Allocation completed']);
    await client.query('COMMIT'); return updated.rows[0];
  } catch(e){ await client.query('ROLLBACK'); throw e; } finally { client.release(); }
}
function auditActor(){ return null; }
